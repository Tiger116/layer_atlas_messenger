    ...
    public synchronized void refresh() {
        ...
        (new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                final List<Participant> participants = new ArrayList<>();  // = mCallback.getAllParticipants();
                    ...
                    public void done(List<ParseObject> parseObjects, ParseException e) {
                        if (e == null) {
                            for (ParseObject user : parseObjects) {
                                ...
                                if (user.get("login") != null && !user.get("login").toString().isEmpty())
                                    participant.login = user.get("login").toString();
                                participant.firstName = user.get("firstName").toString();
                                if (user.get("lastName") != null && !user.get("lastName").toString().isEmpty())
                                    participant.lastName = user.get("lastName").toString();
                                if (user.get("email") != null && !user.get("email").toString().isEmpty())
                                    participant.email = user.get("email").toString();
                                participant.userId = user.getObjectId();
                                participants.add(participant);
                            }
                            if (!participants.isEmpty()) set(participants);
                        } else
                            e.printStackTrace();
                    }
                });
    ...
    private boolean load() {
        ...
        try {
            ...
            for (int i = 0; i < contactsJson.length(); i++) {
                ...
                participant.login = contactJson.optString("login");
                participant.email = contactJson.optString("email");
                participants.add(participant);
            }
    ...
    private boolean save() {
        ...
        try {
            ...
            for (Participant participant : participants) {
                ...
                contactJson.put("login", participant.login);
                contactJson.put("email", participant.email);
