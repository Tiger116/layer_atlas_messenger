 ...
 */
package com.layer.quick_start_android.contacts_utils.async_task_thread_pool;
...
/**
 * <p>
 * CustomAsyncTask enables proper and easy use of the UI thread. This class allows to perform background operations and
 * publish results on the UI thread without having to manipulate threads and/or handlers.
 * </p>
 * <p>
 * CustomAsyncTask is designed to be a helper class around {@link Thread} and {@link Handler} and does not constitute a
 * generic threading framework. CustomAsyncTasks should ideally be used for short operations (a few seconds at the
 * most.) If you need to keep threads running for long periods of time, it is highly recommended you use the various
 * APIs provided by the <code>java.util.concurrent</code> pacakge such as {@link Executor}, {@link ThreadPoolExecutor}
 * and {@link FutureTask}.
 * </p>
 * <p>
 ...
 * </div> <h2>Usage</h2>
 * <p>
 * CustomAsyncTask must be subclassed to be used. The subclass will override at least one method (
 ...
 * </p>
 * <ul>
 * <li>The CustomAsyncTask class must be loaded on the UI thread. This is done automatically as of
 ...
 * <p>
 * CustomAsyncTask guarantees that all callback calls are synchronized in such a way that the following operations are
 ...
 * <p>
 * When first introduced, CustomAsyncTasks were executed serially on a single background thread. Starting with
...
public abstract class AsyncTaskEx<Params, Progress, Result> {
    ...
    /**
     * An {@link java.util.concurrent.Executor} that executes tasks one at a time in serial order. This serialization is global to a
     * particular process.
     */
     ...
     * passed to {@link #execute} by the caller of this task. This method can call {@link #publishProgress} to publish
    ...
    /**
     * <p>
     * Attempts to cancel execution of this task. This attempt will fail if the task has already completed, already been cancelled, or could not be cancelled for some other reason. If successful, and this task has not started when <tt>cancel</tt> is called, this task should never run. If the task has already started, then the <tt>mayInterruptIfRunning</tt> parameter determines whether the thread executing this task should be interrupted in an attempt to stop the task.
     * </p>
     * <p>
     ...
     * @throws IllegalStateException If {@link #getStatus()} returns either {@link lb.listviewvariants.utils.async_task_thread_pool.AsyncTaskEx.Status#RUNNING} or {@link lb.listviewvariants.utils.async_task_thread_pool.AsyncTaskEx.Status#FINISHED}.
     ...
     * reference to it.
     * <p/>
     * This method is typically used with {@link #THREAD_POOL_EXECUTOR} to allow multiple tasks to run in parallel on a pool of threads managed by CustomAsyncTask, however you can also use your own {@link java.util.concurrent.Executor} for custom behavior.
     * <p/>
